...
```
cat kafka-messages | docker run -i -a stdin wurstmeister/kafka /opt/kafka_2.12-0.11.0.0/bin/kafka-console-producer.sh --broker-list $DOCKER_HOST_IP:9092 --topic customer
cat kafka-messages | docker run -i -a stdin wurstmeister/kafka /opt/kafka_2.12-0.11.0.0/bin/kafka-console-producer.sh --broker-list $DOCKER_HOST_IP:9092 --topic audit
```
